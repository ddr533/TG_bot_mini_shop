...
def process_user_handlers(dp: Dispatcher, user_dict: dict, orders_dict: dict):
    ...
    async def process_start_command(message: Message):
        ...
        if user_id not in user_dict:
            user_dict.setdefault(user_id, {'Наименование': None, 'Количество': 0, 'Телефон': None, 'Адрес': None})
        keyboard = create_inline_kb(1, 'but_1', 'but_2', 'but_3', 'but_4', 'but_5')
        await message.answer(text=commands_text['/start'], reply_markup=keyboard)

    @dp.message_handler(CommandHelp())
    async def process_help_command(message: Message):
        keyboard = create_inline_kb(1, 'but_1', 'but_2', 'but_3', 'but_4', 'but_5')
        await message.answer(text=commands_text['/help'], reply_markup=keyboard)

    @dp.message_handler(commands=['cancel'])
    async def process_cancel_command(message: Message):
        keyboard = create_inline_kb(1, 'but_1', 'but_2', 'but_3', 'but_4', 'but_5')
        await message.answer(text=commands_text['/cancel'], reply_markup=ReplyKeyboardRemove())

    @dp.callback_query_handler()
    async def push_product_button(callback: CallbackQuery):
        if callback.from_user.id in user_dict:
            user_dict[callback.from_user.id]['Наименование'] = product_buttons[callback.data]
        await dp.bot.send_message(chat_id = callback.message.chat.id, text=f'Вы выбрали {product_buttons[callback.data]}\n'
    ...
    async def get_phone(message: Message):
        ...
        await dp.bot.send_message(chat_id = message.chat.id, text=f'<b>Телефон</b>: {message.text}\n'
